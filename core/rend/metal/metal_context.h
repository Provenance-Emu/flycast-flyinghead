...
public:
...
#ifdef LIBRETRO
    /// Get current frame buffer data for libretro (BGRA format)
    bool GetFrameBuffer(std::vector<u32>& data, int width, int height);
#endif
    ...
    bool hasPerPixel() override {
        // Metal supports OIT through programmable blending and imageBlocks
        // Enable for iOS 11.0+ and macOS 10.13+ which support Metal 2.0
        return true;
    }
    bool recreateSwapChainIfNeeded();

#ifdef LIBRETRO
    /// Update the Metal layer with new dimensions (for libretro frontend integration)
    void UpdateLayerSize(u32 width, u32 height);
#endif
...
#ifdef LIBRETRO
    id<MTLTexture> offscreenTexture = nil;  // Offscreen render target for libretro mode
#endif
