...
{
#ifdef LIBRETRO
    // In libretro mode, skip Metal framebuffer rendering
    // The frontend handles all rendering through its own Metal video driver
    ...
    else
    {
        ...
        int width;
        int height;
        ReadFramebuffer<RGBAPacker>(info, pb, width, height);
...
{
    // TODO: Implement OIT (Order Independent Transparency) for Metal
    // For now, fall back to regular Metal renderer
    return new MetalRenderer();
}
